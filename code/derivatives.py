...
if __name__=="__main__":
    ...
    if delta > 0: psuff = "%s_vp%02d/"%(param, delta*100)
    else: psuff = "%s_vm%02d/"%(param, abs(delta)*100)
    outfolder += psuff
...
##
#        halocat = BigFileCatalog(scratchcm+ '/fastpm_%0.4f/'%aa, dataset='LL-0.200')
        #print(halocat.attrs)
#        mp = halocat.attrs['M0'][0]*1e10##
        halocat['Mass'] = halocat['Length'].compute() * mp
        cencat = BigFileCatalog(scratchcm+'/fastpm_%0.4f/cencat'%aa+suff)
        satcat = BigFileCatalog(scratchcm+'/fastpm_%0.4f/satcat'%aa+suff)
        rsdfac = read_conversions(scratchcm+'/fastpm_%0.4f/'%aa)
        #
        #

        HImodelz = HImodel(aa)
        HImodelz.derivate(param=param, delta=delta)
