...
#Global, fixed things
myscratch = '/global/cscratch1/sd/chmodi/m3404/martin/'
dpath = '/global/cscratch1/sd/mwhite/PUMA/cyril/HiddenValley//'
project = myscratch
...
#aafiles = aafiles[4:]
#aafiles    = [0.2857,0.3333, 0.3619, 0.4000, 0.4500, 0.5000]
aafiles    = [0.5000, 0.6667, 0.4000]
zzfiles = [round(tools.atoz(aa), 2) for aa in aafiles]

#Paramteres
#Maybe set up to take in as args file?
bs, nc, ncsim, sim, prefix = 1024, 1024, 10240, 'HV%d-F_halos'%10240, ''
bs, nc, ncsim, sim, prefix = 1024, 1024, 10240, 'HV%d-R_halos'%10240, ''
...
def make_galcat(aa, mmin, m1f, alpha=-1, censuff=None, satsuff=None, ofolder=None, seed=3333):
    ...
    halocat = BigFileCatalog(dpath + sim + '_%0.4f/'%aa)
    rank = halocat.comm.rank

    halocat.attrs['BoxSize'] = [bs, bs, bs] #np.broadcast_to(halocat.attrs['BoxSize'], 3)
    halocat.attrs['NC'] = nc

    ghid = halocat.Index.compute()
    halocat['GlobalIndex'] = ghid
    mp = halocat.attrs['M0']*1e10
